...
func buildClientForm(app *tview.Application) tview.Primitive {
	...
	form.AddButton("Connect", func() {
		if serverURL == "" {
			modal := tview.NewModal().SetText("Server URL is required").
				AddButtons([]string{"OK"}).
				SetDoneFunc(func(i int, l string) { app.SetRoot(form, true) })
			app.SetRoot(modal, true)
			return
		}
		root, err := buildChatUI(app, serverURL)
		if err != nil {
			modal := tview.NewModal().
...
func buildChatUI(app *tview.Application, serverURL string) (tview.Primitive, error) {
	...
	msgView := tview.NewTextView().
		...
		SetScrollable(true).
		SetChangedFunc(func() { app.Draw() })
	msgView.SetBorder(true).SetTitle(" Chat ")

	input := tview.NewInputField().SetLabel("Message: ")
	status := tview.NewTextView().
		SetText(fmt.Sprintf("Connected to %s | Esc: back | Ctrl+C: quit", serverURL)).
		SetTextColor(tview.Styles.SecondaryTextColor)
	...
	go func() {
		...
		for {
			...
			if err := json.Unmarshal(data, &incoming); err != nil {
				client.appendLine("[red]decode error:[-] " + err.Error())
				continue
			}
			client.appendLine(fmt.Sprintf("[%s] %s: %s", incoming.Time.Format("15:04:05"), incoming.Username, incoming.Content))
		}
	}()
	...
	input.SetDoneFunc(func(key tcell.Key) {
		...
		outgoing := Message{Username: username, Content: text, Time: time.Now()}
		payload, err := json.Marshal(outgoing)
		if err != nil {
			client.appendLine("[red]encode error:[-] " + err.Error())
			return
		}
