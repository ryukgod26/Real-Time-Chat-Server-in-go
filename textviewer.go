...
func buildChatUI(app *tview.Application, serverURL, username, _ string) (tview.Primitive, error) {
	...
	status := tview.NewTextView().
		...
		SetTextColor(tview.Styles.InverseTextColor)
	...
	input.SetDoneFunc(func(key tcell.Key) {
		...
		payload := Message{Username: username, Content: text, Time: time.Now()}
		msg, err := json.Marshal(payload)
		if err != nil {
			client.appendLine("[red]marshal error:[-] " + err.Error())
			return
		}
	...
	joinMsg, _ := json.Marshal(Message{Username: username, Content: "[joined]"})
